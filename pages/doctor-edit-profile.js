...
// !! Useffect dependenciesi güncelle mesajlarda sıkıntı yaratıyor
const EditProfile = () =>{
    //Local states
    ...
    useEffect(() => {
      ...
      }, [currentUser, idImageUrl, isFaceSimilar, isPersonId, doctorData, personImageUrl, dispatch]);
      ...
      if(idImageUrl && isId){
        const worker = createWorker({
         ...
         // Here we check if the contents of the ID match with the information provided by the doctor before hand
        (async () => {
          await worker.load();
          ...
          if(isId){
            if(text.includes(doctorData.card_id)){
              setIsPersonId(true)
              setSentForReading(false)
              setSuccess(true)
    ...
    return Object.keys(doctorData).length === 0 ? "Yükleniyor" : <IntlProvider locale={language} messages={message[language]} >
    {doctorData.verified ? <h1> <FormattedMessage id="doctor_id_confirmed_message" defaultMessage="default" values={{language}} /> </h1> 
    :
    <>
      <h1> <FormattedMessage id="user_edit_profile_heading" defaultMessage="default" values={{language}} /> </h1>
    <p> <FormattedMessage id="doctor_edit_profile_explanation" defaultMessage="default" values={{language}} /> </p>
    ...
    <div id="fields" >
      ...
      <input  
        ...
        success ?  <FormattedMessage id={resultMessage} defaultMessage="Default" values={{language}} /> : ""
        }
